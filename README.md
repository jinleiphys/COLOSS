...
│   └── ...                # Other modules
├── build/                  # Build artifacts (*.o, *.mod files, generated)
├── adyo_v1_0/             # C++ Coulomb wave function library
│   ├── *.cpp, *.H         # C++ source files
│   └── libcwf_cpp.a       # Compiled C++ library (generated by make)
...
│   └── fort.*             # Output files (generated)
├── COLOSS                  # Compiled executable (generated by make)
├── Makefile               # Build configuration
├── compile.sh             # Automated compilation script
├── run_coloss_gui.sh      # GUI launcher (generated by compile.sh)
└── README.md              # This file
```
...
- **GUI launcher**: `./run_coloss_gui.sh` (auto-generated by compile.sh)

**Note**: The `run_coloss_gui.sh` script is automatically generated by `compile.sh` and is not included in the git repository. It contains dynamic paths that work on any system where you run compile.sh.
...
- **Architecture mismatch (macOS)**: If you see linking errors with `libcwf_cpp.a`, ensure gfortran and g++ are the same architecture:
  ```bash
  gfortran -dumpmachine  # Should show x86_64 or arm64
  g++ -dumpmachine       # Should match gfortran
  ```
  Fix: Install matching compilers via `brew install gcc` or use conda to install architecture-specific versions
