        module scatt_method4
c-----------------------------------------------------------------------
c       Module for Method 4: Scattering using Baye's exact D and T matrices
c
c       This is identical to Method 3 except it uses Baye's analytical
c       formulas for the D (first derivative) and T (second derivative)
c       matrices from Section 3.4.5 of Baye (2015).
c
c       Method 3 uses: barycentric D1 + D2 = D1*D1
c       Method 4 uses: Baye's exact D and T formulas
c
...
c-----------------------------------------------------------------------
            ...
            real*8, dimension(:), allocatable :: leg4_x      ! Mesh points in (0,1)
            real*8, dimension(:), allocatable :: leg4_r      ! Mesh points in (0,R)
            real*8, dimension(:), allocatable :: leg4_w      ! Integration weights

            ! Derivative matrices from Baye (2015) exact formulas
            real*8, dimension(:,:), allocatable :: D1_mat4   ! First derivative d/dr
            real*8, dimension(:,:), allocatable :: D2_mat4   ! Second derivative d²/dr²

            contains

c-----------------------------------------------------------------------
            subroutine init_legendre_mesh_shifted4(N, R)
...
c-----------------------------------------------------------------------
            ...
            if (allocated(leg4_r)) deallocate(leg4_r)
            if (allocated(leg4_w)) deallocate(leg4_w)
            if (allocated(leg4_x)) deallocate(leg4_x)
            allocate(leg4_r(N), leg4_w(N), leg4_x(N))
            ...
            do i = 1, N
                leg4_x(i) = (t_gauss(i) + 1.d0) / 2.d0
                ! Gauss weight on (0,1): lambda_i/2
                leg4_w(i) = w_gauss(i) / 2.d0
            end do
...
c-----------------------------------------------------------------------
            subroutine generate_derivative_matrices_baye4(N, R)
c           Generate derivative matrices for STANDARD Lagrange basis
c           using barycentric formula, then compute EFFECTIVE D2 for
c           x-regularized basis.
c
c           Step 1: Build standard D1 using barycentric formula
c           D1_{i≠j} = w_j / (w_i * (r_i - r_j))
c           D1_{ii} = -sum_{k≠i} D1_{ik}
c
c           Step 2: Build standard D2 = D1 * D1
c
c           Step 3: Compute EFFECTIVE D2 for x-regularized basis:
c           D2_eff_{ij} = 2*D1_{ij}/r_j + (r_i/r_j)*D2_{ij}
c
c           This is the correct formula when using basis functions
c           regularized at origin: f_j(r) ~ r * L_j(r)
c
c           Reference: Baye (2015), Section 3.4.5
c
c           INPUT:
...
c-----------------------------------------------------------------------
            ...
            integer :: i, j, kk
            real*8 :: bary_w(N), prod
            real*8, allocatable :: D1_std(:,:), D2_std(:,:)

            if (allocated(D1_mat4)) deallocate(D1_mat4)
            if (allocated(D2_mat4)) deallocate(D2_mat4)
            allocate(D1_mat4(N, N), D2_mat4(N, N))
            allocate(D1_std(N, N), D2_std(N, N))

            ! ============================================================
            ! Step 1: Compute barycentric weights
            ! w_i = 1 / prod_{k≠i} (r_i - r_k)
            ! ============================================================
            do i = 1, N
                prod = 1.d0
                do kk = 1, N
                    if (kk /= i) then
                        prod = prod * (leg4_r(i) - leg4_r(kk))
                    endif
                end do
                bary_w(i) = 1.d0 / prod
            end do
            ...
            ! ============================================================
            ! Step 2: Build STANDARD D1 matrix using barycentric formula
            ! D1_{i≠j} = w_j / (w_i * (r_i - r_j))
            ! D1_{ii} = -sum_{k≠i} D1_{ik}  (row sum = 0)
            ! ============================================================
            D1_std = 0.d0
            do i = 1, N
                do j = 1, N
                    if (i /= j) then
                        D1_std(i,j) = bary_w(j) / (bary_w(i) *
     &                               (leg4_r(i) - leg4_r(j)))
                    endif
                end do
                ...
                do kk = 1, N
                    if (kk /= i) then
                        D1_std(i,i) = D1_std(i,i) - D1_std(i,kk)
                    endif
                end do
            end do

            ! ============================================================
            ! Step 3: Build STANDARD D2 = D1 * D1
            ! ============================================================
            D2_std = matmul(D1_std, D1_std)

            ! ============================================================
            ! Step 4: Compute EFFECTIVE D2 for x-regularized basis
            ! D2_eff_{ij} = 2*D1_{ij}/r_j + (r_i/r_j)*D2_{ij}
            !
            ! This accounts for the transformation from standard basis
            ! L_j(r) to x-regularized basis f_j(r) = alpha_j * r * L_j(r)
            ! ============================================================
            do i = 1, N
                do j = 1, N
                    D2_mat4(i,j) = 2.d0 * D1_std(i,j) / leg4_r(j)
     &                          + (leg4_r(i) / leg4_r(j)) * D2_std(i,j)
                end do
            end do

            ! Keep standard D1 for boundary condition (we use standard
            ! Lagrange extrapolation at boundary)
            D1_mat4 = D1_std

            deallocate(D1_std, D2_std)

            end subroutine

c-----------------------------------------------------------------------
            subroutine compute_gamma_s4(k_wave, rN, l, gamma_s, eta_som)
...
c-----------------------------------------------------------------------
     ...
     &                  fcp_loc, gcp_loc, kfn, ifail)
            if (ifail /= 0) then
                write(*,*) 'compute_gamma_s4: COUL90 ifail=', ifail
            endif
            ...
            if (abs(hhat) < 1.d-20) then
                write(*,*) "Warning: H^+ very small in compute_gamma_s4"
...
c-----------------------------------------------------------------------
            subroutine solve_scatt_method4(ich, para)
c           Solve scattering problem using Baye's exact D and T matrices
c
c           This is identical to Method 3 except for the derivative matrices.
c-----------------------------------------------------------------------
            use slove_eigen
            ...
            call init_legendre_mesh_shifted4(N_leg, R_outer)

            ! Generate derivative matrices using exact Baye formulas
            call generate_derivative_matrices_baye4(N_leg, R_outer)
            ...
            eta_real = real(eta)
            call compute_gamma_s4(k, R_outer, l, gamma_s, eta_real)
            ...
            ! ============================================================
            ! COMPUTE DERIVATIVE AT BOUNDARY r=R (using Lagrange extrapolation)
            ! ============================================================
            do jr = 1, N_leg
                    ...
                    if (i /= jr) then
                        ...
                        do ir = 1, N_leg
                            if (ir /= jr .and. ir /= i) then
                                prod = prod * (R_outer - leg4_r(ir)) /
     &                                        (leg4_r(jr) - leg4_r(ir))
                            endif
                        end do
                        D1_at_R(jr) = D1_at_R(jr) +
     &                    prod / (leg4_r(jr) - leg4_r(i))
                    endif
                end do
            end do
     ...
     &                      kfn_mesh, ifail_mesh)
                ...
                call compute_potential_at_r4(leg4_r(ir), para, ich, vmod)
                ...
                do jr = 1, N_leg
                    ! Second derivative: d²/dr² (using Baye T matrix)
                    M_matrix(ir, jr) = D2_mat4(ir, jr)
     ...
     &                      dble(l*(l+1)) / leg4_r(ir)**2
            ...
            do jr = 1, N_leg
                ...
                do i = 1, N_leg
                    if (i /= jr) then
                        L_j_at_R = L_j_at_R * (R_outer - leg4_r(i)) /
     &                                        (leg4_r(jr) - leg4_r(i))
                    endif
                end do
...
c-----------------------------------------------------------------------
            subroutine compute_potential_at_r4(r, para, ich, V_short)
...
c-----------------------------------------------------------------------
            ...
            call eval_nuclear_potential4(r, para, ich, V_nuc_r)

            ! Finite-size Coulomb potential
            call eval_coulomb_potential4(r, z12, rrc, V_coul_r)
...
c-----------------------------------------------------------------------
            subroutine eval_nuclear_potential4(r, para, ich, V_nuc)
...
c-----------------------------------------------------------------------
            subroutine eval_coulomb_potential4(r, z12, rc, V_coul)
